...
function TransactionReport() {
  ...
  useEffect(() => {
    const newDt = overviewCardLists.map((list) => {
      ...
      } else if (list.text === "Transactions Volume") {
        return {
          text: "Transactions Volume",
          value: rowData.length,
          icon: <SVGS.TransactionIcon />,
  ...
  const fetchAllMembersInCreditUnion = async () => {
    const response = await FetchCreditUnionMembers(organizationData.value);
  ...
  const fetchTransactions = async () => {
    ...
    if (payload.memberId) {
      ...
      pl.sendingOrganizationId = organizationData.value;
      pl.receivingOrganizationId = organizationData.value;
    }
    if (payload.fromDate) {
      if (payload.toDate) {
    ...
    if (payload.transactionType) {
      ...
      if (typeLists.length === 1 && typeLists[0] !== "") {
        ...
        // pl.transactionType = "ALL_TRANSACTIONS";
      }
    }
    // if (payload.transactionType === "") {
    //   delete pl.startDate;
      ...
      <div className="w-full sm:w-[calc(100%_-_230px)] flex flex-col sm:flex-row items-start h-screen bg-white">
        ...
        <div className="middle-content mt-10 sm:mt-0 w-full sm:w-[calc(100%_-_240px)] p-6">
          <div className="pt-5">
            ...
            <p className="text-sm">
              <span className="text-gray-400">Credit Union</span>:{" "}
              {organizationData?.label}
            </p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 mt-4 gap-4">
            {overviewCardLists.map(
              ({ text, value, icon, showDetails }, index) => (
                <div
                  onClick={showDetails}
